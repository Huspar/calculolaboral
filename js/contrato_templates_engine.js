...
El Trabajador desempeñará sus funciones desde su domicilio particular individualizado en la comparecencia. La jornada semanal será de 42 HORAS. Se estipula el derecho obligatorio de desconexión de 12 horas continuas. El Empleador proveerá las herramientas computacionales y asumirá los costos de conexión e insumos mediante una asignación compensatoria de gastos de teletrabajo de $[35.000] pesos mensuales no imponibles.`
    ...
    // ENSAMBLADOR LEGAL DEL CONTRATO DINÁMICO
    function generarContratoCompleto(datos) {
        ...
        if (datos.tipoContrato === 'plazo') {
            clausulaDuracion = `El presente contrato comenzará a regir con fecha ${escapeHtml(datos.fechaInicio || 'de ingreso')} y tendrá una duración a PLAZO FIJO hasta el día ${escapeHtml(datos.fechaTerminoPlazo || '[Fecha Término]')}, fecha en la cual terminará en forma automática y de pleno derecho de conformidad con el Artículo 159 Nº 4 del Código del Trabajo, sin necesidad de aviso previo ni derecho a indemnización por años de servicio, salvo que las partes acuerden expresamente su prórroga por escrito.`;
        } else if (datos.tipoContrato === 'obra') {
            clausulaDuracion = `El presente contrato es POR OBRA O FAENA DETERMINADA al tenor del Artículo 159 Nº 5 del Código del Trabajo, y se celebra exclusivamente para la ejecución de las labores encomendadas en la obra o faena denominada "${escapeHtml(datos.nombreObra || 'Obra Principal')}", ubicada en ${escapeHtml(datos.ubicacionObra || ciudad)}. El contrato terminará de pleno derecho al concluir de manera natural las funciones específicas del Trabajador o al terminarse la etapa para la cual fue contratado.`;
        } else {
            clausulaDuracion = `El presente contrato comenzará a regir a contar del día ${escapeHtml(datos.fechaInicio || 'de su suscripción')} y tendrá una vigencia de carácter INDEFINIDO, pudiendo terminar únicamente por las causales legales contempladas en el Código del Trabajo.`;
        }
        ...
        if (datos.tipoGratificacion === 'art47') {
            textoGratif = `Las partes convienen el sistema de gratificación anual conforme al Artículo 47 del Código del Trabajo, liquidándose y pagándose anualmente de acuerdo con las utilidades líquidas que arroje el balance comercial auditado.`;
        } else if (datos.tipoGratificacion === 'sin_gratif') {
            textoGratif = `Las partes dejan constancia que la remuneración pactada no contempla anticipo mensual de gratificación legal, rigiéndose las eventuales utilidades por las normas generales supletorias del Código del Trabajo.`;
        } else {
            // Art. 50 (Estándar recomendado DT)
            textoGratif = `El Empleador dará cumplimiento a la obligación legal de gratificar a sus trabajadores mediante el sistema contemplado en el Artículo 50 del Código del Trabajo, pagando mensualmente el 25% de la remuneración mensual devengada, con el tope legal de 4,75 Ingresos Mínimos Mensuales (IMM) prorrateados al mes ($219.115 en 2026).`;
        }

        // Comisiones y semana corrida
        var textoComisiones = '';
        if (datos.tieneComisiones) {
            textoComisiones = `
            <p><strong>Comisiones y Semana Corrida (Art. 45):</strong> Además del sueldo base, el Trabajador devengará una comisión variable consistente en ${escapeHtml(datos.detalleComision || 'un porcentaje convenido sobre las ventas netas efectivamente facturadas y percibidas')}. De conformidad con el Artículo 45 del Código del Trabajo, el Trabajador tendrá derecho al pago de la SEMANA CORRIDA por los días domingos y festivos devengados en el período, calculada en base al promedio diario de las comisiones devengadas en el respectivo mes calendario.</p>`;
        }
        ...
        if (colacion > 0 || movilizacion > 0 || teletrabajoAsig > 0) {
            ...
            if (colacion > 0) textoAsignaciones += ` Asignación de Colación: ${formatCLP(colacion)} mensuales;`;
            if (movilizacion > 0) textoAsignaciones += ` Asignación de Movilización: ${formatCLP(movilizacion)} mensuales;`;
            if (teletrabajoAsig > 0) textoAsignaciones += ` Asignación de Teletrabajo / Conectividad: ${formatCLP(teletrabajoAsig)} mensuales;`;
            textoAsignaciones += ` las que no constituirán remuneración para ningún efecto legal ni previsional.</p>`;
        }

        // 5. Cláusulas de Blindaje Opcionales
        var clausulaLeyKarin = `
        <div class="clausula-block">
            <h4 class="clausula-titulo">SÉPTIMO: COMPROMISO Y PROTOCOLO LEY KARIN (LEY Nº 21.643 Y DS 44 MINTRAB)</h4>
            <p>El Empleador y el Trabajador asumen el compromiso recíproco de mantener un ambiente de trabajo libre de cualquier manifestación de violencia, acoso laboral y acoso sexual. El Trabajador declara recibir en este acto copia íntegra del Protocolo de Prevención de la Violencia y Acoso Laboral regulado por el Decreto Supremo Nº 44 del Ministerio del Trabajo. El incumplimiento de estas normas o la realización de actos de acoso debidamente comprobados será calificado como falta gravísima para todos los efectos legales, habilitando la terminación inmediata del contrato conforme al Artículo 160 Nº 1 letras b) y f) del Código del Trabajo.</p>
        </div>`;

        var clausulaConfidencialidad = datos.incluirConfidencialidad !== false ? `
        <div class="clausula-block">
            <h4 class="clausula-titulo">OCTAVO: CONFIDENCIALIDAD, PROPIEDAD INTELECTUAL Y NO CONCURRENCIA</h4>
            <p>El Trabajador se obliga a guardar estricto secreto y reserva sobre toda información técnica, códigos fuente, bases de datos de clientes, tarifas y secretos comerciales a los que acceda con motivo de sus servicios. Asimismo, todas las creaciones, inventos, metodologías, softwares o diseños que el Trabajador desarrolle durante su jornada serán de propiedad exclusiva y total del Empleador. Durante la vigencia del contrato, el Trabajador no podrá realizar negociaciones ni prestar servicios dentro del mismo giro de la empresa, sea por cuenta propia o de terceros competidores.</p>
        </div>` : '';

        var clausulaHerramientas = `
        <div class="clausula-block">
            <h4 class="clausula-titulo">NOVENO: HERRAMIENTAS DE TRABAJO Y DEBER DE RESTITUCIÓN</h4>
            <p>Los equipos computacionales, teléfonos móviles, vehículos, herramientas manuales, tarjetas o vestimenta que el Empleador proporcione al Trabajador son para uso exclusiva y estrictamente laboral. El Trabajador se compromete a su adecuado cuidado y custodia, debiendo restituirlos en buen estado (salvo el desgaste por uso legítimo) de manera inmediata al momento de terminar la relación laboral o cuando el Empleador así lo requiera.</p>
        </div>`;

        var clausulaBandasHorarias = datos.incluirBandaHoraria ? `
        <div class="clausula-block">
            <h4 class="clausula-titulo">DÉCIMO: BANDA HORARIA PARA TRABAJADORES MADRES Y PADRES (ART. 27 LEY 40 HORAS)</h4>
            <p>De conformidad con el Artículo 27 del Código del Trabajo incorporado por la Ley Nº 21.561, y habiendo acreditado el Trabajador ser madre/padre/cuidador de un menor de 12 años mediante el respectivo certificado de nacimiento, las partes pactan una banda horaria de hasta dos horas, pudiendo el Trabajador anticipar o retrasar su ingreso diario en hasta 60 minutos, debiendo compensar proporcionalmente dicho tiempo al término de la jornada diaria.</p>
        </div>` : '';

        var clausulaMiDT = `
        <div class="clausula-block">
            <h4 class="clausula-titulo">DÉCIMO PRIMERO: REGISTRO ELECTRÓNICO LABORAL (PORTAL MI DT)</h4>
            <p>En cumplimiento de lo dispuesto en el Artículo 9º bis del Código del Trabajo y el Decreto Nº 37 del Ministerio del Trabajo, el Empleador registrará este contrato individual de trabajo en el sitio web de la Dirección del Trabajo (portal Mi DT) dentro del plazo improrrogable de quince días corridos contados desde su suscripción.</p>
        </div>`;

        var clausulaEjemplares = `
        <div class="clausula-block">
            <h4 class="clausula-titulo">DÉCIMO SEGUNDO: EJEMPLARES Y DOMICILIO</h4>
            <p>Para todos los efectos legales derivados del presente contrato, las partes fijan su domicilio en la comuna de ${escapeHtml(ciudad)} y se someten a la competencia de sus Tribunales de Letras del Trabajo. El presente contrato se firma en dos ejemplares del mismo tenor y fecha, declarando el Trabajador haber recibido en este acto un ejemplar de su texto original.</p>
        </div>`;
    ...
    function generarHTMLVisual(res, esVistaPrevia) {
        ...
        <div class="contrato-a4-document ${marcaAguaClase}">
            ${esVistaPrevia ? '<div class="watermark-contrato">VISTA PREVIA DE EJEMPLO • CÁLCULO LABORAL CHILE</div>' : ''}
            ...
            <div class="contrato-cuerpo text-xs leading-relaxed text-slate-800 space-y-3.5 text-justify">
                <p>
                    En <strong>${escapeHtml(res.ciudad)}</strong>, a <strong>${escapeHtml(res.fecha)}</strong>, entre:
                </p>
                ...
                <div class="clausula-block">
                    <h4 class="clausula-titulo">PRIMERO: CARGO, SERVICIOS Y REGLAMENTO INTERNO</h4>
                    <p>
                        El Trabajador se compromete a prestar sus servicios personales bajo subordinación y dependencia en el cargo de 
                        <strong>${escapeHtml(d.cargoNombre || '[Nombre del Cargo]')}</strong>, debiendo desempeñar con lealtad, esmero y eficiencia las siguientes funciones:
                    </p>
                    <ul class="list-disc pl-5 mt-1.5 space-y-1 text-slate-700">
                        ${res.listaFuncionesHtml}
                    </ul>
                    <p class="mt-1.5">
                        Asimismo, el Trabajador se compromete a acatar estrictamente las órdenes que le impartan sus superiores jerárquicos y cumplir las normas del Reglamento Interno de Orden, Higiene y Seguridad de la empresa, del cual declara recibir copia formal en este acto.
                    </p>
                </div>

                <div class="clausula-block">
                    <h4 class="clausula-titulo">SEGUNDO: LUGAR DE PRESTACIÓN DE LOS SERVICIOS</h4>
                    <p>
                        ...
                        De conformidad con el Artículo 12 del Código del Trabajo, el Empleador podrá alterar la naturaleza de los servicios o el sitio o recinto en que ellos deban prestarse, a condición de que se trate de labores similares, que el nuevo sitio quede dentro de la misma localidad o ciudad y sin que ello importe menoscabo económico ni moral para el Trabajador.
                    </p>
                </div>

                <div class="clausula-block">
                    <h4 class="clausula-titulo">TERCERO: JORNADA DE TRABAJO (LEY Nº 21.561 DE 40 HORAS / 42H VIGENTES)</h4>
                    <p>${res.textoJornada.replace(/\n/g, '<br>')}</p>
                </div>

                <div class="clausula-block">
                    <h4 class="clausula-titulo">CUARTO: REMUNERACIÓN Y PERIODICIDAD DE PAGO</h4>
                    <p>
                        El Empleador pagará al Trabajador una remuneración mensual compuesta por:
                    </p>
                    <p class="pl-3 mt-1">
                        a) <strong>Sueldo Base Mensual:</strong> La suma fija e imponible de <strong>${formatCLP(res.sueldoBase)} CLP</strong> mensuales.
                    </p>
                    <p class="pl-3 mt-1">
                    ...
                    <p class="mt-1.5">
                        Las remuneraciones se liquidarán y pagarán por mensualidades vencidas el último día hábil de cada mes calendario, mediante transferencia bancaria a la cuenta del Trabajador o pago en dinero efectivo contra firma de la liquidación de sueldo respectiva (Art. 54 del Código del Trabajo).
                    </p>
                </div>

                <div class="clausula-block">
                    <h4 class="clausula-titulo">QUINTO: DURACIÓN Y VIGENCIA DEL CONTRATO</h4>
                    <p>${res.clausulaDuracion}</p>
                </div>

                <div class="clausula-block">
                    <h4 class="clausula-titulo">SEXTO: PREVISIÓN SOCIAL Y DESCUENTOS LEGALES</h4>
                    <p>
                        De las remuneraciones devengadas por el Trabajador, el Empleador retendrá y pagará en los organismos de seguridad social respectivos (Previred) las cotizaciones previsionales obligatorias de cargo del trabajador: cotización para pensión en su Administradora de Fondos de Pensiones (AFP), cotización legal del 7% para salud (Fonasa o Isapre), Seguro de Cesantía (AFC) en la proporción que legalmente le corresponda y los impuestos que graven la renta (Art. 58 del Código del Trabajo).
                    </p>
                </div>
                ...
                <div class="pt-10 mt-8 grid grid-cols-2 gap-8 text-center text-xs">
                    <div>
                        <div class="border-t border-slate-400 pt-2 font-bold text-slate-900 uppercase">
                    ...
                    <div>
                        ...
                        <div class="text-[11px] text-slate-600 font-mono">
                            RUT: ${escapeHtml(d.trabajadorRut || 'XX.XXX.XXX-X')}
                        </div>
                        <div class="text-[10px] text-slate-500">
                ...
                <div class="page-break-before mt-12 pt-6 border-t-2 border-dashed border-slate-300">
                    <div class="text-center mb-4">
                        <span class="text-[9.5px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                            Documento Anexo Obligatorio (Decreto Supremo Nº 44 Mintrab)
                        </span>
                        <h3 class="text-base font-extrabold text-slate-900 mt-1 uppercase">
                            Constancia de Entrega y Recepción de Protocolo Ley Karin
                        </h3>
                    </div>
                    <p class="text-[11px] leading-relaxed text-slate-700 text-justify">
                        En este mismo acto, don(ña) <strong>${escapeHtml(d.trabajadorNombre || '[Nombre Trabajador]')}</strong>, 
                        RUT <strong>${escapeHtml(d.trabajadorRut || '[RUT]')}</strong>, declara bajo fe de juramento haber recibido de parte de su empleador 
                        <strong>${escapeHtml(d.empresaRazonSocial || '[Empleador]')}</strong> un ejemplar impreso y digital del:
                    </p>
                    <ol class="list-decimal pl-5 mt-2 space-y-1 text-[11px] text-slate-800 font-semibold">
                        <li>Reglamento Interno de Orden, Higiene y Seguridad actualizado conforme a la Ley Nº 21.643.</li>
                        <li>Protocolo Institucional de Prevención, Denuncia e Investigación del Acoso Sexual, Laboral y la Violencia en el Trabajo (DS 44).</li>
                        <li>Canales y formularios oficiales para la presentación de denuncias ante la empresa o la Dirección del Trabajo.</li>
                    </ol>
                    <div class="pt-8 grid grid-cols-2 gap-8 text-center text-[11px]">
                        <div>
                            <div class="border-t border-slate-300 pt-1 font-bold text-slate-800">Firma Empleador</div>
                        </div>
                        <div>
                            <div class="border-t border-slate-300 pt-1 font-bold text-slate-800">Firma y Huella Trabajador(a)</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    // EXPORTADOR A WORD (.DOCX COMPATIBLE HTML FORMAT)
    function descargarWordDoc(datos) {
        var res = generarContratoCompleto(datos);
        var visualHtml = generarHTMLVisual(res, false); // false = sin marca de agua
        ...
        <head>
            ...
            <style>
                @page {
                    size: 21.59cm 27.94cm; /* Carta o A4 */
    ...
    function imprimirPDF(datos, esVistaPrevia) {
        ...
        ventana.document.write(`
            ...
            <head>
                ...
                <style>
                    ...
                    @media print {
                        ...
                        .contrato-a4-document { box-shadow: none !important; border: none !important; padding: 0 !important; width: 100% !important; max-width: 100% !important; }
                    }
            ...
            <body>
                <div class="no-print max-w-[820px] mx-auto mb-4 flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200">
                    <span class="text-xs text-slate-600 font-semibold">Contrato generado para <strong>${escapeHtml(datos.trabajadorNombre || 'Trabajador')}</strong></span>
                    <div class="flex gap-2">
                        ...
                        <button onclick="window.close()" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs rounded-lg cursor-pointer">
                            Cerrar
                        </button>
                    </div>
                </div>
